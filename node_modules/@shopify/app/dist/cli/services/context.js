...
const appFromId = async (appId, token) => {
    ...
    if (!app)
        throw new BugError(`Couldn't find the app with Client ID "${appId}". ${resetHelpMessage}`);
    return app;
};
...
export async function fetchOrCreateOrganizationApp(app, token) {
...
export async function fetchAppAndIdentifiers(options, token) {
    ...
    if (options.reset) {
        ...
        if (!partnersApp) {
            throw new AbortError(outputContent `Couldn't find the app with Client ID ${apiKey}`, outputContent `• If you didn't intend to select this app, run ${outputContent `${outputToken.packagejsonScript(options.app.packageManager, 'deploy', '--reset')}`.value}`);
        }
    }
    else {
