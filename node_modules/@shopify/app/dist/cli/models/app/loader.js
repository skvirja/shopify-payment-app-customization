...
export async function findSpecificationForConfig(specifications, configurationPath, abortOrReport) {
    ...
    if (!specification) {
        ...
        abortOrReport(outputContent `Unknown extension type ${outputToken.yellow(type)} in ${outputToken.path(configurationPath)}. ${isShopifolk ? shopifolkMessage : ''}`, undefined, configurationPath);
        return undefined;
    }
...
class AppLoader {
    ...
    async loaded() {
        ...
        const appClass = new App(name, 'SHOPIFY_API_KEY', this.appDirectory, packageManager, configuration, configurationPath, nodeDependencies, webs, allExtensions, usesWorkspaces, dotenv);
    ...
    async loadExtensions(extensionDirectories) {
        ...
        const extensions = configPaths.map(async (configurationPath) => {
            ...
            const specification = await findSpecificationForConfig(this.specifications, configurationPath, this.abortOrReport.bind(this));
            if (!specification)
                return;
...
export function getAppConfigurationFileName(config) {
    if (config) {
